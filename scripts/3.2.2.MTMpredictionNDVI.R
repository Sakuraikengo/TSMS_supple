...
mtmFolder <- "2019_M100_Xacti4eyeCamera_Images/result/3.2.2.MTMpredictionNDVI"
...
if (length(list.files(mtmFolder)) < 7) {
  ...
  resultAll <- foreach(dayInd = 1:length(dataWeek), .packages = c("MTM", "corrplot")) %dopar% {
    ...
    for (treatment_i in name_treatment) {
      ...
      # make the matrix for the result
...
resultCsvMT2File <- "2019_M100_Xacti4eyeCamera_Images/result/3.2.1.MTMpredictionWithFlowerDate/resultLongDf.csv"
