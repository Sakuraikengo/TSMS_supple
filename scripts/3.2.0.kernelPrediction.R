...
allResult <- foreach(dayInd = 1:length(dataWeek), .packages = c("BGLR", "RAINBOWR", "stringr")) %do% { 
  # dayInd <- 1
  ...
  foreach(conditionEach = condition, .packages = c("BGLR", "RAINBOWR", "stringr")) %dopar% { 
    ...
    ZETAList <- list(list(ZETA = ZETA_G,  X0 = NULL, nameInd = "G"), 
                     list(ZETA = ZETA_MS, X0 = NULL, nameInd = "MS"), 
                     list(ZETA = ZETA_GM, X0 = NULL, nameInd = "G+MS"), 
                     list(ZETA = ZETA_GF, X0 = NULL, nameInd = "G+Field"))
...
ggLineList <- lapply(resultIndList, function(eachResultInd) {
  ...
  g <- ggplot(allDayResultEach, aes(x = Week, y = value, colour = Model, linetype = Model)) + 
    ...
    scale_color_manual(values = c("steelblue2", "ivory4", "springgreen3", "tan1")) + 
