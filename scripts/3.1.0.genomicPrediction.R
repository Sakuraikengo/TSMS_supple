...
kernelFolder <- "2019_M100_Xacti4eyeCamera_Images/result/3.1.0.genomicPrediction"
...
# "G+Flower" is a genomic prediction model including Flowering or not as fixed effect
kernelName <- c("G", "G+Flower")
resultIndex <- c("Correlation", "R2", "RMSE")

# make the array to input correlation between observed data and predicted data
...
# # read the reference data and check the number of accessions used in this analysis
# # The below code is only for checking
...
allResult <- foreach(dayInd = 1:length(dataWeek), .packages = c("BGLR", "RAINBOWR", "stringr")) %dopar% { 
  ...
  # make the save folder for each date
  ...
  # take log for "dryWeight" and "plantArea", 
  # ensuring that the error term follows a normal distribution
  ...
  for (conditionEach in condition) {
    ...
    # extract the phenotypic data for each condition
    ...
    eachResult <- lapply(ZETAList, function(eachList) {
      ...
      for (seedIndEach in 1:length(seedInd)) {
        ...
        for (times in 1:5) {
          ...
          # for the linear prediction model
          if (ZETA == "lm") {
